...
def test(
    ...
    ckpt_emo = {
        k.replace('module.', ''): v for k, v in ckpt_emo.items()
    }
    ...
    emo_mapping.load_state_dict(
        ckpt_emo_mapping['emo_mapping_state_dict'],
    )
    ...
    if test_mode == 'random':
        random_images = random.sample(range(1, 2), 1)

        for image_number in range(len(random_images)):

            image_id = str(random_images[image_number]).zfill(6)

            latent_path = images_path + image_id + ".npy"
        ...
        for v_idx in tqdm(range(len(valence)), desc="Valences "):
            ...
            for a_idx in range(len(arousal)):
                ...
                emotion = torch.FloatTensor(
                    [valence[v_idx], arousal[a_idx]]
                )\
                    .float().unsqueeze_(0).to(device)
                ...
                emos_data[(valence[v_idx], arousal[a_idx])].append(
                    (
                        abs(emos[0] - valence[v_idx]),
                        abs(emos[1] - arousal[a_idx])
                    )
                )
                ...
                ax_g[iter].set_title(
                    "V: p{:.2f}, r{:.2f}, A: p{:.2f}, r{:.2f}".format(
                        valence[v_idx],
                        emos[0],
                        arousal[a_idx],
                        emos[1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--valence", type=float, nargs='+', default=[-1, 1])
    parser.add_argument("--arousal", type=float, nargs='+', default=[-1, 1])
